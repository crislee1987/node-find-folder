...
_cs = lazypipe().pipe($.plumber).pipe(function() {
  return $["if"](clp.coffeelint, _coffeelint());
}).pipe($.coffee, cfg.cs_opts);

gulp.task('coffeescript', function() {
  var ff_src, gulpfile_src, test_src;
  ...
  ff_src.pipe($.changed(cfg.path.project_root)).pipe(_cs()).pipe($.rename({
    dirname: '',
    basename: 'index',
    extname: '.js'
  })).pipe(gulp.dest(cfg.path.project_root));
  test_src.pipe($.changed(cfg.path.test)).pipe(_cs()).pipe(gulp.dest(cfg.path.test));
  gulpfile_src.pipe($.changed(cfg.path.project_root)).pipe(_cs()).pipe(gulp.dest(cfg.path.project_root));
});
